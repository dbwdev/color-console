...
class GoColorProvider implements vscode.DocumentColorProvider {
  public provideDocumentColors(
    ...
    while ((m = pattern.exec(text))) {
      b.push(m[1]);
      const positionStart = document.positionAt(
  ...
  public provideColorPresentations(
    ...
    return new Promise(resolve => {
      resolve(
        b.map(el => {
          const bb = `#${toHex(color.red * 255)}${toHex(
            color.green * 255
...
function insertConsoleLog(type) {
  ...
  const whiteSpacesNumber = Math.max(
    thisLine.firstNonWhitespaceCharacterIndex,
  ...
  let insertText =
    type === 'primitive'
      ? `\n${spaceee}console.log('%c${sss}', 'color: ${newColor()}', '${selectedText} ->', ${selectedText});`
      : `\n${spaceee}console.log('%c⧭', 'color: ${newColor()}', '${selectedText} ->', ${selectedText});`;
  // console.log(document.languageId === 'vue')
  // console.log(insertText)
  if (document.languageId === 'vue') insertText = insertText.slice(0, -1);
  activeEditor
    .edit(eb => eb.insert(endOfThisLine, insertText))
    .then(() => {
      if (selectedText) return;
      nextLine = document.lineAt(selection.end.translate(1, 0).line);
      const endOfNextLine = new vscode.Position(
        selection.end.translate(1, 0).line,
        nextLine.range.end.character - (document.languageId === 'vue' ? 1 : 2)
      );
...
export function activate(ctx: vscode.ExtensionContext): void {
  ctx.subscriptions.push(
    vscode.commands.registerCommand('extension.coloredPrimitives', () =>
      insertConsoleLog('primitive')
    )
  );
  ctx.subscriptions.push(
    vscode.commands.registerCommand('extension.coloredObject', () =>
      insertConsoleLog('object')
    )
  );
  ctx.subscriptions.push(
